...
namespace M3H5Lib.Api
{
	...
	public partial class CRTVSUMIResource : M3BaseResourceEndpoint
	{
		...
		/// Version Release: 14.x
		/// </summary>
		/// <param name="m3XALT">Alternative Search Key (Required)</param>
			...
			CancellationToken cancellationToken = default)
		{
			...
			{
				EndpointUri = $"{Config.BasePath}/{ProgramName}/GetSupplier",
			};

			// Validate mandatory parameters
			if (string.IsNullOrWhiteSpace(m3XALT))
				throw new ArgumentNullException(nameof(m3XALT));

			// Set mandatory parameters
			request
				.WithQueryParameter("XALT", m3XALT.Trim());
		...
		/// <param name="m3CONO">Company</param>
		/// <param name="m3DIVI">Division</param>
		/// <param name="m3XALT">Alternative Search Key</param>
		/// <param name="maxRecords">Maximum number of records to return</param>
		/// <param name="includeMetadata">Include Metadata records in response (default false)</param>
		/// <param name="trimResults">Trim result data (default true)</param>
		/// <param name="outputColumns">List of columns to return, null or empty for all columns</param>
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (!string.IsNullOrWhiteSpace(m3DIVI))
				request.WithQueryParameter("DIVI", m3DIVI.Trim());
			if (!string.IsNullOrWhiteSpace(m3XALT))
				request.WithQueryParameter("XALT", m3XALT.Trim());
