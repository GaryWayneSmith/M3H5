...
namespace M3H5Lib.Api
{
	...
	/// Description: Intrastat trade statistics interface
	/// Version Release: 5e90
	///</summary>
	public partial class TXS300MIResource : M3BaseResourceEndpoint
	{
		public TXS300MIResource(ApiClient apiClient)
			: base(apiClient)
		{
			ProgramName = "TXS300MI";
		...
		/// <param name="m3VRNL">Tax declaration number (Required)</param>
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (string.IsNullOrWhiteSpace(m3VRNL))
				throw new ArgumentNullException(nameof(m3VRNL));

			// Set mandatory parameters
			request
				.WithQueryParameter("DIVI", m3DIVI.Trim())
				.WithQueryParameter("VRNL", m3VRNL.Trim());
		...
		/// <param name="m3RRLN">Report reference line</param>
		/// <param name="maxRecords">Maximum number of records to return</param>
		/// <param name="includeMetadata">Include Metadata records in response (default false)</param>
		/// <param name="trimResults">Trim result data (default true)</param>
		/// <param name="outputColumns">List of columns to return, null or empty for all columns</param>
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (m3RRLN.HasValue)
				request.WithQueryParameter("RRLN", m3RRLN.Value.ToString(CultureInfo.CurrentCulture));
